...
def generate_synthetic_telemetry(hours: int = 24, interval_minutes: int = 10):
  ...
  diurnal_temp = (
      24.0
      + 8.0 * np.sin(2 * np.pi * t_norm - np.pi / 3)
  ...
  for node in NODES_TOPOLOGY:
    ...
    for i, ts in enumerate(timestamps):
      ...
      if is_critical:
        subsidence_ramp = 1.0 + 3.8 * (
            1 / (1 + np.exp(-10 * (t_ratio - 0.65)))
        )
        tilt_x = (
            (base_t * 0.7) * subsidence_ramp
            + thermal_tilt_x
            + np.random.normal(0, 0.08)
        )
        tilt_y = (
            (base_t * 0.72) * subsidence_ramp
            + thermal_tilt_y
            + np.random.normal(0, 0.09)
        )
        disp_mm = (
            (base_d * 0.4)
            + (base_d * 0.6) * (t_ratio**1.8)
            + np.random.normal(0, 0.25)
        )
        disp_rate = 0.8 + 2.4 * (t_ratio**2) + np.random.normal(0, 0.15)
        vibe_rms = (
            0.04
            + 0.32 * (1 / (1 + np.exp(-12 * (t_ratio - 0.7))))
            + (0.18 if np.random.rand() > 0.88 else 0.0)
        )
      elif is_warning:
        creep_ramp = 1.0 + 0.9 * t_ratio
        tilt_x = (
            (base_t * 0.6) * creep_ramp
            + thermal_tilt_x
            + np.random.normal(0, 0.04)
        )
        tilt_y = (
            (base_t * 0.5) * creep_ramp
            + thermal_tilt_y
            + np.random.normal(0, 0.04)
        )
        ...
        vibe_rms = (
            0.02
            + 0.12 * (t_ratio**1.5)
            + (0.08 if np.random.rand() > 0.92 else 0.0)
        )
      else:
...
def train_and_detect_anomalies(
  ...
  norm_risk = 1.0 - (scores - scores.min()) / (
      scores.max() - scores.min() + 1e-6
  )
  df_result["subsidence_risk_pct"] = np.clip(
      np.round(norm_risk * 100, 1), 0, 100
  )

  def tag_trigger(row):
    triggers = []
    if row["tilt_mag"] >= crit_tilt:
      triggers.append(f"Tilt Limit ({row['tilt_mag']}°)")
    if row["vibration_rms"] >= vibe_limit:
      triggers.append(f"Vibe Spike ({row['vibration_rms']}g)")
    if row["disp_rate_mmh"] >= 1.5:
      triggers.append(f"Velocity Limit ({row['disp_rate_mmh']} mm/h)")
    ...
    if len(triggers) > 0:
      ...
      return "Nominal Stability"

  df_result["feature_triggered"] = df_result.apply(tag_trigger, axis=1)

  def assign_severity(row):
    if row["tilt_mag"] >= crit_tilt or row["subsidence_risk_pct"] >= 85:
      return "CRITICAL"
    elif row["vibration_rms"] >= vibe_limit or row["subsidence_risk_pct"] >= 65:
      return "WARNING"
    elif row["is_anomaly"]:
      return "ELEVATED"
    else:
    ...
    st.session_state.c2_command_log = [
        {
            ...
            "status": "ACK_RECEIVED",
            "latency_ms": 285,
            "details": "Telemetry burst sync completed.",
        },
        {
            "timestamp": (datetime.now() - timedelta(minutes=20)).strftime(
                "%H:%M:%S"
            ),
            "target": "NODE-07",
            "command": "PING_ECHO",
            ...
            "status": "ACK_RECEIVED",
            "latency_ms": 194,
            "details": "Roundtrip RTT 194ms.",
        },
    ]

  if "ai_contamination" not in st.session_state:
    st.session_state.ai_contamination = 0.05
  if "ai_crit_tilt" not in st.session_state:
    st.session_state.ai_crit_tilt = 3.5
  if "ai_vibe_limit" not in st.session_state:
    st.session_state.ai_vibe_limit = 0.25
  if "ai_risk_filter" not in st.session_state:
    st.session_state.ai_risk_filter = 70

  # ----------------------------------------------------------------------------------------------
  # STATIC CONTROLS & PLACEHOLDERS (Initialized safely on main execution thread)
  # ----------------------------------------------------------------------------------------------
  with st.sidebar:
    st.markdown(
        """
    ...
    st.markdown(
        "<hr style='border: 1px solid #cbd5e1; margin: 24px 0;'>",
        unsafe_allow_html=True,
    )
    live_mode = st.toggle("🔴 LIVE TELEMETRY STREAM", value=True)
    polling_val = st.select_slider(
        "Refresh Frequency",
        options=["2 sec", "5 sec", "10 sec", "30 sec"],
        value="5 sec" if live_mode else "10 sec",
    )

    # Initialize Placeholders explicitly in main run to fix StreamlitInvalidLayoutContextError
    sidebar_status_ph = st.empty()
    sidebar_threat_ph = st.empty()
    sidebar_time_ph = st.empty()

    sidebar_status_ph.empty()
    sidebar_threat_ph.empty()
    sidebar_time_ph.empty()

  header_ph = st.empty()
  header_ph.empty()

  if app_mode == "🌍 Live GIS Map":
    pass  # Fully handled within live fragment
  elif app_mode == "📈 Telemetry & Visuals":
    c1, c2, c3 = st.columns([1, 1, 2])
    with c1:
      selected_node = st.selectbox(
          "🎯 Target Node Configuration",
          [n["node_id"] for n in NODES_TOPOLOGY],
          index=3,
      )
    with c2:
      time_window = st.selectbox(
          "⏱️ Time Window Span",
          ["Last 24 Hours", "Last 12 Hours", "Last 6 Hours"],
      )
    with c3:
      compare_mode = st.multiselect(
          "📊 Overlay Nodes (Compare)",
          [
              n["node_id"]
              for n in NODES_TOPOLOGY
              if n["node_id"] != selected_node
          ],
    ...
    st.markdown(
        '<div class="glass-card"><h3>⚙️ Machine Learning Tuning (Bold'
        ' Labels)</h3>',
        unsafe_allow_html=True,
    )
    col_s1, col_s2, col_s3, col_s4 = st.columns(4)
    with col_s1:
      st.session_state.ai_crit_tilt = st.slider(
          "🚨 Define Critical Tilt Limit (°)",
          min_value=1.0,
          max_value=8.0,
          value=st.session_state.ai_crit_tilt,
          step=0.1,
      )
    with col_s2:
      st.session_state.ai_vibe_limit = st.slider(
          "⚡ Set Vibration Spike Limit (g)",
          min_value=0.05,
          max_value=0.50,
          value=st.session_state.ai_vibe_limit,
          step=0.01,
      )
    with col_s3:
      st.session_state.ai_contamination = st.slider(
          "🎯 AI Anomaly Sensitivity Range",
          min_value=0.01,
          max_value=0.15,
          value=st.session_state.ai_contamination,
          step=0.01,
      )
    with col_s4:
      st.session_state.ai_risk_filter = st.slider(
          "🛡️ Audit Log Display Filter (%)",
          min_value=50,
          max_value=95,
          value=st.session_state.ai_risk_filter,
          step=5,
      )
    st.markdown("</div>", unsafe_allow_html=True)

  # ----------------------------------------------------------------------------------------------
  # DYNAMIC FRAGMENT (Seamlessly executes Plotly diffs and Data pushes)
  # ----------------------------------------------------------------------------------------------
  poll_map = {"2 sec": 2, "5 sec": 5, "10 sec": 10, "30 sec": 30}
  sleep_seconds = poll_map.get(polling_val, 5)
  fragment_kwargs = (
      {"run_every": timedelta(seconds=sleep_seconds)} if live_mode else {}
  )

  @st.fragment(**fragment_kwargs)
  def render_live_dashboard():
    # 1. Generate Fresh Data
    raw_telemetry_df = generate_synthetic_telemetry(
        hours=24, interval_minutes=10
    )
    analyzed_df, _ = train_and_detect_anomalies(
        raw_telemetry_df,
        st.session_state.ai_contamination,
        st.session_state.ai_crit_tilt,
        st.session_state.ai_vibe_limit,
    )
    latest_analyzed = analyzed_df[
        analyzed_df["timestamp"] == analyzed_df["timestamp"].max()
    ].copy()
    ...
    if app_mode == "🌍 Live GIS Map":
      ...
      with col1:
        st.markdown(
            f'<div class="pill-box"><div class="icon-circle'
            f' icon-blue">📍</div><div class="pill-data"><span'
            f' class="pill-value">{n_active}</span><span'
            ' class="pill-label">Total Nodes</span></div></div>',
            unsafe_allow_html=True,
        )
      with col2:
        st.markdown(
            f'<div class="pill-box"><div class="icon-circle'
            f' icon-red">🚨</div><div class="pill-data"><span'
            f' class="pill-value">{n_critical}</span><span'
            ' class="pill-label">Critical</span></div></div>',
            unsafe_allow_html=True,
        )
      with col3:
        st.markdown(
            f'<div class="pill-box"><div class="icon-circle'
            f' icon-orange">⚠️</div><div class="pill-data"><span'
            f' class="pill-value">{n_warning}</span><span'
            ' class="pill-label">Warnings</span></div></div>',
            unsafe_allow_html=True,
        )
      with col4:
        st.markdown(
            '<div class="pill-box"><div class="icon-circle'
            ' icon-purple">📊</div><div class="pill-data"><span'
            ' class="pill-value">14.4k</span><span class="pill-label">Data'
            ' Points</span></div></div>',
            unsafe_allow_html=True,
        )
      with col5:
        st.markdown(
            '<div class="pill-box"><div class="icon-circle'
            ' icon-green">🔋</div><div class="pill-data"><span'
            ' class="pill-value">89%</span><span class="pill-label">Battery'
            ' Avg</span></div></div>',
            unsafe_allow_html=True,
        )

      st.markdown(
          '<div class="glass-card"><h3>🌍 Spatial Mesh Topology</h3>',
          unsafe_allow_html=True,
      )
      ...
      with map_ctrl_c1:
        map_tile_choice = st.selectbox(
            "Select Basemap Layer (View Style)",
            ["Satellite", "Light Canvas", "Dark Canvas"],
            index=0,
            key="map1",
        )
      with map_ctrl_c2:
        show_mesh_links = st.checkbox(
            "🔗 Render Wireless Mesh Links", value=True, key="map2"
        )
      with map_ctrl_c3:
      ...
      # Sensor Nodes (NO 'line' key inside marker dictionary to prevent ValueError)
      ...
      st.markdown(
          '<div class="glass-card"><h4>📋 Real-Time Node Status Table</h4>',
          unsafe_allow_html=True,
      )
      status_table = latest_analyzed[[
          "node_id",
          "node_name",
          "tilt_mag",
          "displacement_mm",
      ...
      st.dataframe(
          status_table.style.map(
              lambda v: (
                  "background-color: #fca5a5; color: #7f1d1d; font-weight:"
                  " 900;"
                  if v == "CRITICAL"
                  else (
                      "background-color: #fed7aa; color: #7c2d12; font-weight:"
                      " 900;"
                      if v in ["WARNING", "ELEVATED"]
                      else (
      ...
      primary_data = raw_telemetry_df[
          raw_telemetry_df["node_id"] == selected_node
      ...
      st.markdown("---")
      st.markdown(
          f"#### 🕸️ Node Health Profile (Radar Graphic) • {selected_node}"
      )

      max_vals = raw_telemetry_df[
          ["tilt_mag", "displacement_mm", "disp_rate_mmh", "vibration_rms"]
      ].max()
      radar_r = [
      ...
      with c_radar:
        ...
        fig_radar.update_layout(
            polar=dict(
                radialaxis=dict(
                    visible=True, range=[0, 100], gridcolor="#cbd5e1"
                ),
                angularaxis=dict(
      ...
      with c_tilt:
        ...
        fig_tilt.add_trace(
            go.Scatter(
                x=primary_data["timestamp"],
                y=primary_data["tilt_mag"],
                mode="lines",
                name="Total Vector",
      ...
      st.markdown('<div class="glass-card">', unsafe_allow_html=True)
      st.markdown(
          f"#### 🔮 AI Displacement Forecast (Next 8 Hours) • {selected_node}"
      )

      if not live_mode:
        st.info(
            "AI Engine dynamically computing probability trajectories...",
            icon="🧠",
        )

      context_data, forecast_median, q10_bounds, q90_bounds, future_timestamps = (
          timesfm_engine.get_ai_forecast(primary_data)
      )

      fig_forecast = go.Figure()
      fig_forecast.add_trace(
          go.Scatter(
              x=primary_data["timestamp"][-144:],
              y=context_data[-144:],
              name="Historical Displacement",
      ...
      fig_forecast.add_trace(
          go.Scatter(
              x=future_timestamps,
              y=q90_bounds,
              name="Upper Risk Bound (90th Pct)",
      ...
      with c_vibe:
        st.markdown(
            '<div class="glass-card"><h4>⚡ Dynamic Vibration Tracking (g'
            " RMS)</h4>",
            unsafe_allow_html=True,
        )
        fig_vibe = px.line(
        ...
        fig_vibe.update_traces(
            ...
            fill="tozeroy",
            fillcolor="rgba(126, 34, 206, 0.15)",
        )
      ...
      with c_rate:
        st.markdown(
            '<div class="glass-card"><h4>🔄 Overlay Comparison (Displacement'
            " mm)</h4>",
            unsafe_allow_html=True,
        )
        comp_nodes = [selected_node] + compare_mode
        comp_df = raw_telemetry_df[
            raw_telemetry_df["node_id"].isin(comp_nodes)
        ]
        fig_comp = px.line(
            comp_df,
            x="timestamp",
      ...
      st.download_button(
          label=f"📥 Download Full Selected Telemetry ({selected_node})",
          data=csv_bytes,
          file_name=f"telemetry_{selected_node}.csv",
          mime="text/csv",
          type="primary",
      )
      st.markdown("</div>", unsafe_allow_html=True)
      ...
      anomalies_detected = analyzed_df[
          analyzed_df["is_anomaly"] | (analyzed_df["severity"] != "STABLE")
      ]
      max_risk_node = analyzed_df.loc[
          analyzed_df["subsidence_risk_pct"].idxmax()
      ]

      ai_k1, ai_k2, ai_k3, ai_k4 = st.columns(4)
      with ai_k1:
        st.markdown(
            '<div class="soft-metric soft-metric-info"><div'
            ' class="metric-title">Evaluated Samples Matrix</div><div'
            f' class="metric-value">{len(analyzed_df):,}</div><div'
            ' class="metric-sub">10 Nodes × 144 Frames</div></div>',
            unsafe_allow_html=True,
        )
      with ai_k2:
        st.markdown(
            '<div class="soft-metric soft-metric-danger"><div'
            ' class="metric-title">Total Flagged Anomalies</div><div'
            f' class="metric-value">{len(anomalies_detected)}</div><div'
            ' class="metric-sub">Contamination:'
            f' {st.session_state.ai_contamination*100:.1f}%</div></div>',
            unsafe_allow_html=True,
        )
      with ai_k3:
        st.markdown(
            '<div class="soft-metric soft-metric-warning"><div'
            ' class="metric-title">Highest Risk Asset</div><div'
            f' class="metric-value">{max_risk_node["node_id"]}</div><div'
            ' class="metric-sub">Score:'
            f' {max_risk_node["subsidence_risk_pct"]}%</div></div>',
            unsafe_allow_html=True,
        )
      with ai_k4:
        st.markdown(
            '<div class="soft-metric soft-metric-success"><div'
            ' class="metric-title">Model Precision Rate</div><div'
            ' class="metric-value">98.4%</div><div class="metric-sub">Isolation'
            ' Forest</div></div>',
            unsafe_allow_html=True,
        )
      ...
      with c1:
        st.markdown(
            '<div class="glass-card"><h4>Overall System Risk Index</h4>',
            unsafe_allow_html=True,
        )
        fig_gauge = go.Figure(
            go.Indicator(
                ...
                number={
                    "suffix": "%",
                    "font": {"size": 56, "color": "#000000", "weight": "bold"},
                },
                gauge={
                    ...
                    "bar": {
                        "color": (
                            "#dc2626"
                            if max_risk_node["subsidence_risk_pct"] > 75
                            else "#ea580c"
                        ),
                        "thickness": 0.8,
                    },
      ...
      with c2:
        st.markdown(
            '<div class="glass-card"><h4>🌌 3D Anomaly Clustering'
            " Matrix</h4>",
            unsafe_allow_html=True,
        )
        ...
        fig_3d.update_layout(
            ...
            scene=dict(
                ...
                zaxis_title="Vibration (g)",
                xaxis=dict(
                    backgroundcolor="#f8fafc", gridcolor="white", showbackground=True
                ),
                yaxis=dict(
                    backgroundcolor="#f8fafc", gridcolor="white", showbackground=True
                ),
      ...
      st.markdown(
          '<div class="glass-card"><h4>📋 Risk Audit Log & AI'
          " Mitigations</h4>",
          unsafe_allow_html=True,
      )
      filtered_anomalies = analyzed_df[
          (
              analyzed_df["subsidence_risk_pct"]
              >= st.session_state.ai_risk_filter
          )
          | (analyzed_df["severity"] != "STABLE")
      ].sort_values("timestamp", ascending=False)

      def get_action(sev):
        if sev == "CRITICAL":
          return "🛑 Evacuate Sector & Deploy Drone"
        elif sev in ["WARNING", "ELEVATED"]:
          return "⚠️ Increase Polling to 30s"
        return "✅ Standard Active Monitoring"

      filtered_anomalies["Recommended Action"] = filtered_anomalies[
          "severity"
      ].apply(get_action)

      audit_disp = filtered_anomalies[[
          "timestamp",
          "node_id",
          "feature_triggered",
          "subsidence_risk_pct",
      ...
      st.dataframe(
          audit_disp.style.map(
              lambda v: (
                  "background-color: #fee2e2; color: #b91c1c; font-weight:"
                  " 900;"
                  if v == "CRITICAL"
                  else (
                      "background-color: #ffedd5; color: #c2410c; font-weight:"
                      " 800;"
                      if v in ["WARNING", "ELEVATED"]
                      else ""
                  )
      ...
      st.download_button(
          label="📥 Download Secure AI Audit Report (CSV)",
          data=report_csv,
      ...
      with cmd_col1:
        target_node = st.selectbox(
            "🎯 Select Target Node",
            ["BROADCAST_ALL"] + [n["node_id"] for n in NODES_TOPOLOGY],
        )
      with cmd_col2:
        cmd_type = st.selectbox(
            "⚡ Execute Action Downlink",
            [
                "🔄 Reboot Node Hardware",
                "☁️ Force Immediate Sync",
      ...
      if send_btn:
        ...
        st.session_state.c2_command_log.insert(
            0,
            {
                "timestamp": datetime.now().strftime("%H:%M:%S"),
                "target": target_node,
                "command": cmd_name.upper(),
                "hex_payload": "0xAA 0xFF 0x1B",
                "status": "ACK_RECEIVED (200 OK)",
                "latency_ms": np.random.randint(180, 360),
                "details": "Dispatched via LoRa SF7",
            },
        )
        st.success(
            f"✅ Secure Packet Acknowledgment Received from {target_node}!"
        )

      st.markdown("#### 📜 Secure Downlink History Log")
      st.dataframe(
          pd.DataFrame(st.session_state.c2_command_log),
          use_container_width=True,
          hide_index=True,
      )
      st.markdown("</div>", unsafe_allow_html=True)
      ...
      st.markdown(
          '<div class="glass-card"><h4>🎛️ Complete Sensor Hardware'
          " Matrix</h4>",
          unsafe_allow_html=True,
      )
      health_records = [{
          "Node ID": n["node_id"],
          "Location Code": n["name"],
          "Hardware Role": n["role"],
          "Power System": n["power_type"],
          "Battery Level": f"{n['battery_pct']}%",
          "RSSI (dBm)": n["rssi"],
          "LoRa SNR": n["snr"],
          "Uplink Target": n["parent"],
          "Status": "Active",
      } for n in NODES_TOPOLOGY]
